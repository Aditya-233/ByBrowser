...
pub async fn ensure_engine_running() -> Result<()> {
    if is_local_port_open(1080).await {
        println!(
            "\x1b[92m[✓] DPI bypass proxy is active on 127.0.0.1:1080 (reusing instance)\x1b[0m"
        );
        return Ok(());
    }
    ...
    println!(
        "\x1b[93m[*] Starting integrated SOCKS5 DPI bypass engine on 127.0.0.1:1080...\x1b[0m"
    );
    tokio::spawn(async {
        if let Err(e) = run_socks5_server("127.0.0.1", 1080).await {
            eprintln!(
                "\x1b[91m[!] Embedded SOCKS5 DPI engine exited with error: {:#}\x1b[0m",
                e
            );
        }
    });
    ...
    for _ in 0..25 {
        ...
        if is_local_port_open(1080).await {
            println!(
                "\x1b[92m[✓] Integrated SOCKS5 DPI bypass engine is online (Remote DNS enabled)\x1b[0m"
            );
            return Ok(());
        }
    }
    ...
    eprintln!(
        "\x1b[93m[!] Warning: Embedded engine took longer than expected to bind port 1080\x1b[0m"
    );
    Ok(())
}
...
async fn run_socks5_server(listen_ip: &str, port: u16) -> Result<()> {
    ...
    loop {
        ...
        tokio::spawn(async move {
            ...
            let _ = handle_socks5_client(stream, peer_addr).await;
        });
    }
}
